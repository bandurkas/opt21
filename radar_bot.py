            ...
            if authorized_admins:
                ...
                for opp in opps:
                    ...
                    if sig_id not in sent_signals:
                        ...
                        msg = (
                            f"🚨 *АРБИТРАЖНЫЙ СИГНАЛ (Чистая прибыль: ~${actual_profit:.2f})* 🚨\n\n"
                            ...
                            f"Вам нужно выступить маркет-мейкером. Выставьте лимитный ордер:\n"
                            f"👉 Объём (Size): *{TRADE_SIZE} ETH* (чтобы хватило маржи $200)\n"
                            f"👉 Действие: {opp['action']}\n\n"
                            f"✅ *ШАГ 6 (ХЕДЖ - КАК ЗАБРАТЬ ПРОФИТ):*\n"
                            f"Как только вашу лимитку заберут на {opp['wide_exchange']}, **немедленно** откройте биржу *{opp['tight_exchange']}* и нажмите **SELL (Продать по рынку)** этот же самый опцион с таким же объемом ({TRADE_SIZE} ETH).\n"
                            f"Разница цен останется у вас на балансе!\n\n"
                            f"⏳ *КАК ДОЛГО ЖДАТЬ?*\n"
                            f"Если вашу лимитку на {opp['wide_exchange']} не забрали в течение **3-5 минут**, просто отмените её. Спред мог закрыться, ждем следующий сигнал."
                        )
